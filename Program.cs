...
namespace ScsExtractorGui
{
    public class MainForm : Form
    {
        ...
        private CheckBox chkList, chkListAll, chkListEntries, chkTree;
        ...
        private void InitializeComponents()
        {
            ...
            tabControl.TabPages.AddRange(new[] { basicTab, advancedTab, hashfsTab });
        ...
        private void AddBasicControls(TabPage page)
        {
            ...
            btnBrowse.Click += (s, e) => {
                using (OpenFileDialog ofd = new OpenFileDialog { 
                    Filter = "SCS Files|*.scs|All Files|*.*",
                    Title = "Select SCS File" 
                })
                    if (ofd.ShowDialog() == DialogResult.OK) txtPath.Text = ofd.FileName;
            };
            page.Controls.AddRange(new[] { txtPath, btnBrowse });
            y += 35;
            ...
            page.Controls.AddRange(new[] { txtDest, btnDestBrowse });
            y += 45;
            ...
            page.Controls.AddRange(new[] { rbModeNormal, rbModeList, rbModeListAll });
            y += 30;
            ...
            page.Controls.AddRange(new[] { rbModeListEntries, rbModeTree });
            y += 45;
            ...
            page.Controls.AddRange(new[] { txtPathsFile, btnPathsBrowse });
            y += 45;
            ...
            page.Controls.AddRange(new[] { chkAll, chkSeparate, chkSkip, chkQuiet, chkDryRun });
        }
        ...
        private void AddHashFSControls(TabPage page)
        {
            ...
            page.Controls.AddRange(new[] { txtAdditionalFile, btnAdditionalBrowse });
            y += 45;
        ...
        private async void RunExtractor(object? sender, EventArgs? e)
        {
            ...
            { 
                MessageBox.Show("Please select an input file/folder.", "Missing Input", 
                    MessageBoxButtons.OK, MessageBoxIcon.Warning); 
                return; 
            }
            if (!File.Exists(extractorPath))
            {
                MessageBox.Show("extractor.exe not found!", "Error", 
        ...
        private void RunProcess(string args)
        {
            try
            {
                using (currentProcess = new Process())
                {
                    ...
                    currentProcess.OutputDataReceived += (s, a) => 
                    { 
                        if (a.Data != null) 
                            Log(a.Data); 
                    };
                    currentProcess.ErrorDataReceived += (s, a) => 
                    { 
                        if (a.Data != null) 
                            Log("ERR: " + a.Data); 
                    };
